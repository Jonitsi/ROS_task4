...
class Service(Node):
	def __init__(self):
		...
		self.checkpoint_list = [[2, -6], [5, 5], [-1, 5], [-7, -1], [0,0]] #list of all checkpoints
		self.cp_links = [[0,1],[0,4],[1,2],[2,3],[4,3]] #connections between checkpoints
		self.visited_cps = [] # keeps track of visited checkpoints
		self.adjacency_matrix = np.zeros([len(self.checkpoint_list), len(self.checkpoint_list)]) #adjacency_matrix
		self.weight_matrix = np.zeros([len(self.checkpoint_list), len(self.checkpoint_list)]) #weight matrix
	...
	def plan_callback(self, request, response):
			...
			if len(self.visited_cps) == 0:
				self.visited_cps.append(start_cp)
				print(self.visited_cps)
				pose = PoseStamped()
				pose.pose.position.x = float(start_cp)
			...
			while not q.empty() :
				...
				if current_cp not in self.visited_cps:
				#if len(self.visited_cps) == len(visited):
					print('plan ready, path is: {}' .format(current_path))
					final_path = current_path
					self.visited_cps.append(current_cp)
					break
			...
			for i, checkpoint in enumerate(final_path):
				if i == 0:
					continue
				pose = PoseStamped()
				pose.pose.position.x = float(checkpoint)
				response.plan.poses.append(pose)
