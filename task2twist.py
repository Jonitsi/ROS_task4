		...
		if abs(angle_to_cp - self.theta) > 0.1:
			...
			if self.checkpoint_index < len(self.path):
				...
				if self.checkpoint_index < len(self.path):
					...
					print('Checkpoint ', self.path[self.checkpoint_index - 1], ' reached, next checkpoint: ', self.path[self.checkpoint_index])
				else:
					print('Reached destination')
					
			if self.checkpoint_index > len(self.path) - 1:
				self.twist.linear.x = 0.0
				...
				plt.show()
				#plt.plot(self.twists)
				#plt.show()
				
				self.checkpoint = self.path[self.checkpoint_index]
		else:
